...
<% if (isSourceLandmark(page, $$= checkElem $$)) {
	def _wr_landmarks$$= _wr_tag_index $$ = safeSubList($$ if (nested) { $$getChildLandmarks($$= ancestor $$$$= landmarkTypes $$)$$ } else { $$getAvailableLandmarks(page, $$= level $$$$= landmarkTypes $$)$$ } $$, "$$= tag.range $$")
