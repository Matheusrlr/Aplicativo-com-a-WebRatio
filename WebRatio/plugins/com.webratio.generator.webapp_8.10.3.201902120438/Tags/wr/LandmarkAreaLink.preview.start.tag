...
if (StringUtils.isEmpty(position)) {
    tagProblems.addFatalError("<wr:LandmarkAreaLink>: the position attribute is mandatory")
}
...
$$
[% for ($$= var $$ in safeSubList(getAvailableLandmarks(page, 1, LandmarkType.AREA), "$$= position $$-$$= position $$")) { %]