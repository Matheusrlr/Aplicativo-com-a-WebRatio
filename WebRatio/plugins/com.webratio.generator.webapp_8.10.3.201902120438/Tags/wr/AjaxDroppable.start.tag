...
if (StringUtils.isEmpty(context)) {
    ...
    if (loopTagsSize == 0) {
      tagProblems.addFatalError("<wr:AjaxDroppable/> must be surrounded by one of the following tags: " + names)
    }
...
$$<%printRaw(executeContextTemplate("MVC/Ajax/DroppableGenerator.template", ["element": $$=context$$?.selectSingleNode("$$=selector$$"), "item" : $$printRaw(item)$$, "position" : $$printRaw(position)$$]))%>