...
<% if (isSourceLandmark(page, $$=checkElem$$)) {
	def _wr_landmarks$$= _wr_tag_index $$ = safeSubList($$ if (nested) { $$getChildLandmarks($$=ancestor$$, LandmarkType.ACTION, LandmarkType.FLOATING_EVENT)$$ } else { $$getAvailableLandmarks(page, $$= level $$, LandmarkType.ACTION, LandmarkType.FLOATING_EVENT)$$ } $$, "$$=tag.range$$")
