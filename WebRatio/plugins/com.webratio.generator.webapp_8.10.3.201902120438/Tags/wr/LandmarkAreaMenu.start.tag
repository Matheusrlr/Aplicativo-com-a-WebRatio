...
<% if (isSourceLandmark(page, $$= checkElem $$)) {
	def _wr_landmarks$$= _wr_tag_index $$ = safeSubList($$ if (nested) { $$getChildLandmarks($$= ancestor $$, LandmarkType.AREA)$$ } else { $$getAvailableLandmarks(page, $$= level $$, LandmarkType.AREA)$$ } $$, "$$= tag.range $$")
