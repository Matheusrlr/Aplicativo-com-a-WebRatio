...
if (StringUtils.isEmpty(position)) {
    tagProblems.addFatalError("<wr:LandmarkPageLink>: the position attribute is mandatory")
}
...
$$
[% for ($$= var $$ in safeSubList(getAvailableLandmarks(page, 1, LandmarkType.PAGE), "$$= position $$-$$= position $$")) { %]